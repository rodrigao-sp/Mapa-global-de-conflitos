    ...
    <div className="w-full h-[500px] rounded-lg overflow-hidden relative">
      ...
      >
        ...
        {selectedConflict && coordenadasConflitos[selectedConflict] && (
          <MapCenterUpdater coordinates={coordenadasConflitos[selectedConflict]} />
        )}
        {conflitos.map((conflito) => {
          const coordinates = coordenadasConflitos[conflito.name] || conflito.coordinates;
          return (
              ...
              >
                <Popup>
                  <div className="p-2 transition-all duration-300">
                    ...
                    <p className="text-red-600 font-semibold">{conflito.status}</p>
                    <p className="text-sm mt-1">{conflito.description}</p>
                    <div className="mt-2 text-sm space-y-2">
                      <div className="flex items-center gap-2 transition-all duration-300">
                        <span>Vítimas Fatais: {formatarNúmero(conflito.vítimas)}</span>
                        <TrendIcon trend={conflito.vítimasTendência} />
                      </div>
                      <div className="flex items-center gap-2 transition-all duration-300">
                        <span>Deslocados: {formatarNúmero(conflito.deslocados)}</span>
                        <TrendIcon trend={conflito.deslocadosTendência} />
                      </div>
                      <div className="flex items-center gap-2 transition-all duration-300">
                        <span>Área Afetada: {formatarNúmero(conflito.área)} km²</span>
                      </div>
                      <div className="mt-2">
                        <p className="font-semibold text-sm">Últimos Eventos:</p>
                        <ul className="text-xs space-y-1 mt-1">
                          {conflito.últimosEventos.slice(0, 2).map((evento, idx) => (
                            <li key={idx} className={`
                              py-1 px-2 rounded
                              ${evento.tipo === 'combate' ? 'bg-red-100 text-red-700' :
                                evento.tipo === 'humanitário' ? 'bg-blue-100 text-blue-700' :
                    ...
                    <div className="text-xs text-gray-500 mt-2 flex flex-col gap-1">
                      <p>Início: {new Date(conflito.dataInício).toLocaleDateString('pt-BR')}</p>
                      <p>Atualizado: {new Date(conflito.lastUpdate).toLocaleString('pt-BR')}</p>
                    </div>
                  </div>
                </Popup>
              </Marker>
            ...
            <Popup>
              <div className="p-2 transition-all duration-300">
                ...
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2 transition-all duration-300">
                    <span className="text-sm">Nível de Tensão: {zona.nívelDeTensão.toFixed(1)}/10</span>
                    <TrendIcon trend={zona.tensãoTendência} />
                  </div>
                  <div>
                    <p className="text-sm font-semibold mt-2">Fatores de Risco:</p>
                    <ul className="text-xs space-y-1 mt-1">
                      {zona.fatoresDeRisco.map((fator, idx) => (
                        <li key={idx} className="bg-gray-100 text-gray-700 py-1 px-2 rounded">
                          {fator}
                        </li>
                      ))}
                    </ul>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-1">
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: getCorDeZonaDeRisco(zona.nívelDeTensão)
                        }}
                      />
                    </div>
                    <p className="text-xs text-right mt-1">{zona.probabilidadeConflito.toFixed(1)}%</p>
                  </div>
                  <div className="text-xs text-gray-500 mt-2">
                    <p>Países Envolvidos: {zona.países.join(', ')}</p>
                    <p className="mt-1">Atualizado: {new Date(zona.lastUpdate).toLocaleString('pt-BR')}</p>
                  </div>
                </div>
              </div>
            </Popup>
